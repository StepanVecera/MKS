...
static const uint8_t kod[] = {7, 9, 3, 2, 12};
...
{
  /* USER CODE BEGIN 1 */
	static uint32_t poz = 0;
	static uint32_t t_pos_stisk = 0;
  /* USER CODE END 1 */
  ...
  while (1)
  {
	  if (key != -1)
	  {
		  printf("stisknuto: %d\n", key);
		  HAL_Delay(250);
		  if (HAL_GetTick()-t_pos_stisk <3000)
		  {
			  if (key == kod[poz])
			  {
				  printf("dobre \n");
